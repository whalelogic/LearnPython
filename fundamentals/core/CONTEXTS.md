...
- This file fits naturally with [../../system/FILE_IO_GUIDE.md](../../system/FILE_IO_GUIDE.md), because `with open(...)` is the first context manager most learners use.
- Try rewriting any `open()` example with and without `with` so you can see how context managers reduce cleanup mistakes.
- When the concept clicks, build your own tiny context manager for timing a block of code or printing "starting" and "finished" messages.
