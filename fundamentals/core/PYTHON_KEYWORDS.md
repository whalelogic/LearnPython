...
- Use this keyword reference alongside [../README.md](../README.md) to connect syntax words like `if`, `for`, `return`, and `class` to the beginner-friendly examples in the fundamentals folder.
- When a keyword changes control flow, try building a tiny command-line menu so you can see how `while`, `break`, `continue`, and `match` shape the program.
- Keywords become easier to remember when you group them by purpose: definition (`def`, `class`), branching (`if`, `elif`, `else`), iteration (`for`, `while`), and error handling (`try`, `except`, `finally`).
