...
- Start with the lighter overview in [../functions/Built_In_Functions.md](../functions/Built_In_Functions.md) before using this file as a reference table.
- Compare the examples here with [../../standard_library/Built_In_Functions.md](../../standard_library/Built_In_Functions.md) to see when a built-in function is enough and when you should reach for a module.
- A good practice exercise is to rewrite a small loop-heavy script by replacing manual counters, lookups, and accumulation with `enumerate()`, `zip()`, `sum()`, and `sorted()`.
