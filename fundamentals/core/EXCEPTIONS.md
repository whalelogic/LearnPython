...
- Combine this reference with [../../system/FILE_IO_GUIDE.md](../../system/FILE_IO_GUIDE.md) to practice realistic errors such as missing files, invalid JSON, and permission issues.
- As you work through examples, ask two questions: which exception type actually helps the caller, and what cleanup still needs to happen when something fails?
- A strong beginner exercise is to write a parser that converts user input to integers and gracefully handles both `ValueError` and `KeyboardInterrupt`.
